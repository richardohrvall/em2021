...
#files_bolag <- list.files(here("data/bolag"), pattern = ".xlsx$", full.names = TRUE)

read_files <- . %>% 
  read_xlsx(., skip = 2, sheet = "Antal företag") %>%
  clean_names() %>% 
  {if("radetiketter" %in% names(.)) rename(., kommun = radetiketter) else .} %>%  
  filter(kommun != "Totalsumma") %>% 
  separate(kommun, c("llkk", "kommun"), sep = "\\s", extra = "merge")
